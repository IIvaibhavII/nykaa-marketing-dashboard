...
with t1:
    ...
    with c2:
        ...
        fig.update_layout(**{k:v for k,v in base_layout(h=320).items() if k not in('xaxis','yaxis')},
                          title="Revenue Share",
                          showlegend=False,
                          annotations=[dict(text=f'₹{T_REV/1e9:.1f}B<br><span style="font-size:10px">Total</span>',
                                           x=0.5,y=0.5,font=dict(size=14,color='#eeeeff',family='Sora'),showarrow=False)])
        st.plotly_chart(fig,use_container_width=True)
    ...
    with c5:
        ...
        for _,r in ch.iterrows():
            ...
            fig.add_trace(go.Scatterpolar(
                ...
                line=dict(color=fpal[r['Campaign_Type']],width=2),
                fillcolor=fpal[r['Campaign_Type']].replace(')',',0.1)').replace('rgb','rgba') if 'rgb' in fpal[r['Campaign_Type']] else fpal[r['Campaign_Type']]+'22',
                opacity=0.9,
            ))
        fig.update_layout(
...
with t5:
    ...
    fig.update_layout(
        **{k:v for k,v in base_layout(h=440).items() if k not in('xaxis','yaxis')},
        ...
        showlegend=True,
        annotations=[dict(
            text='<b style="font-size:15px">Budget<br>Split</b>',
            x=0.5,y=0.5,
