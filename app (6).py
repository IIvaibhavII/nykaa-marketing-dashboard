...
def indian_grouping(n):
    """Format an integer with Indian digit grouping: 12345678 -> 1,23,45,678"""
    n = int(round(n))
    sign = '-' if n < 0 else ''
    s = str(abs(n))
    if len(s) <= 3:
...
def inr(n, decimals=2):
    """Rupee value in Cr / Lakh / Thousand, Indian-context short form."""
    n = float(n); sign = '-' if n < 0 else ''; a = abs(n)
    if a >= 1e7:  return f"{sign}₹{a/1e7:,.{decimals}f} Cr"
    if a >= 1e5:  return f"{sign}₹{a/1e5:,.{decimals}f} L"
    if a >= 1e3:  return f"{sign}₹{a/1e3:,.1f} K"
    return f"{sign}₹{a:,.0f}"

def inr_full(n):
    """Full precision rupee value with Indian comma grouping."""
    return f"₹{indian_grouping(n)}"

def num(n, decimals=2):
    """Plain count in Cr / Lakh / Thousand, Indian-context short form."""
    n = float(n); sign = '-' if n < 0 else ''; a = abs(n)
    if a >= 1e7:  return f"{sign}{a/1e7:,.{decimals}f} Cr"
    if a >= 1e5:  return f"{sign}{a/1e5:,.{decimals}f} L"
    if a >= 1e3:  return f"{sign}{a/1e3:,.1f} K"
...
# ── Icon system (clean line-icon SVGs, replaces emoji) ──
ICONS = {
    'revenue':  '<path d="M6 3h9l3 3v15H6z"/><path d="M9 3v18M9 8h6M9 13h6"/>',
    'trend-up': '<polyline points="22 7 13.5 15.5 8.5 10.5 2 17"/><polyline points="16 7 22 7 22 13"/>',
    'target':   '<circle cx="12" cy="12" r="9"/><circle cx="12" cy="12" r="5"/><circle cx="12" cy="12" r="1.4"/>',
    'wallet':   '<path d="M20 7H4a2 2 0 0 0-2 2v9a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2V9a2 2 0 0 0-2-2Z"/><path d="M16 5H4"/><circle cx="16.5" cy="13.5" r="1.4"/>',
    'eye':      '<path d="M1.5 12S5.5 5 12 5s10.5 7 10.5 7-4 7-10.5 7S1.5 12 1.5 12Z"/><circle cx="12" cy="12" r="3"/>',
    'cursor':   '<path d="M5 3l14 6-6 2-2 6z"/>',
    'bolt':     '<polygon points="12.5 2 4 14 11 14 10.5 22 20 10 13 10 12.5 2"/>',
    'magnet':   '<circle cx="12" cy="12" r="9"/><path d="M12 7v10M7 12h10" stroke-width="1.6"/>',
    'trophy':   '<circle cx="12" cy="8.5" r="5.5"/><path d="M14.8 13.2 16 21l-4-2.4L8 21l1.2-7.8"/>',
    'coins':    '<circle cx="9" cy="9" r="6.5"/><path d="M13 6.4A6.5 6.5 0 1 1 13 17.6"/>',
    'users':    '<circle cx="9" cy="8" r="3.2"/><path d="M3.5 19a5.5 5.5 0 0 1 11 0"/><path d="M15.5 6a3.2 3.2 0 0 1 0 6.2"/><path d="M17 12.5a5 5 0 0 1 3.8 4.9"/>',
    'globe':    '<circle cx="12" cy="12" r="9"/><path d="M3 12h18M12 3c2.6 2.6 4 5.7 4 9s-1.4 6.4-4 9c-2.6-2.6-4-5.7-4-9s1.4-6.4 4-9Z"/>',
    'alert':    '<path d="M12 3 2 20h20L12 3Z"/><path d="M12 10v4.5M12 17.3h.01"/>',
    ...
    'pie':      '<path d="M12 3v9l7.8 4.5A9 9 0 1 1 12 3Z"/><path d="M13 3.1A9 9 0 0 1 20.9 12H13Z"/>',
    'link':     '<path d="M9.5 13.5a4.5 4.5 0 0 0 6.8.5l3-3a4.5 4.5 0 0 0-6.4-6.4l-1.6 1.6"/><path d="M14.5 10.5a4.5 4.5 0 0 0-6.8-.5l-3 3a4.5 4.5 0 0 0 6.4 6.4l1.6-1.6"/>',
    'refresh':  '<path d="M21 8a9 9 0 0 0-15-6.7L3 4"/><path d="M3 4v5h5"/><path d="M3 16a9 9 0 0 0 15 6.7L21 20"/><path d="M21 20v-5h-5"/>',
    'calendar': '<rect x="3" y="4.5" width="18" height="16" rx="2"/><path d="M16 2.5v4M8 2.5v4M3 10h18"/>',
    'tools':    '<path d="M14.7 6.3 17.7 3.3a5 5 0 0 1 1.8 6.9l-8.6 8.6a3 3 0 0 1-4.2-4.2l8.6-8.6a5 5 0 0 1 6.9 1.8L19 10.7"/>',
    'sparkle':  '<path d="M12 3v5M12 16v5M3 12h5M16 12h5M5.6 5.6l3 3M15.4 15.4l3 3M5.6 18.4l3-3M15.4 8.6l3-3"/>',
    'pulse':    '<polyline points="22 12 18 12 15 20 9 4 6 12 2 12"/>',
    'bag':      '<path d="M6.5 8 4 21h16l-2.5-13Z"/><path d="M9 8V6a3 3 0 0 1 6 0v2"/>',
}

def icon(name, color, size=18, stroke=1.9, bg=True, box=38):
    """Return an HTML span with a clean SVG line-icon, optionally on a soft rounded badge."""
    paths = ICONS.get(name, ICONS['sparkle'])
    svg = (f'<svg width="{size}" height="{size}" viewBox="0 0 24 24" fill="none" '
           f'stroke="{color}" stroke-width="{stroke}" stroke-linecap="round" '
           f'stroke-linejoin="round">{paths}</svg>')
    if not bg:
        return svg
    return (f'<div style="width:{box}px;height:{box}px;border-radius:11px;'
            f'background:{hex_to_rgba(color,0.14)};border:1px solid {hex_to_rgba(color,0.3)};'
...
with st.sidebar:
    ...
    <div style='text-align:center;padding:1.4rem 0 1.6rem'>
        <div style='display:inline-flex;align-items:center;justify-content:center;width:56px;height:56px;
            border-radius:16px;background:linear-gradient(135deg,rgba(255,61,138,0.18),rgba(167,139,250,0.14));
            border:1px solid rgba(255,61,138,0.35);box-shadow:0 0 20px rgba(255,61,138,0.25)'>
            {icon('bag','#ff3d8a',size=26,stroke=1.7,bg=False)}
        </div>
        <div style='font-family:Sora,sans-serif;font-size:1.15rem;font-weight:800;
                    color:#fff;margin:10px 0 3px;letter-spacing:-0.3px'>NYKAA</div>
        <div style='font-size:0.68rem;color:#444466;letter-spacing:2px;text-transform:uppercase'>
            Marketing Intelligence &middot; India</div>
    </div>
    ...
    <div style='margin-top:1.2rem;padding:1rem;background:#0d0d1a;border-radius:12px;
        ...
        <span style='color:#ff3d8a;font-weight:700;letter-spacing:1px'>DATASET</span><br>
        Records &nbsp;·&nbsp; <b style='color:#aab'>{indian_grouping(len(df))}</b><br>
        Channels &nbsp;·&nbsp; <b style='color:#aab'>{df['Campaign_Type'].nunique()}</b><br>
        Period &nbsp;·&nbsp; <b style='color:#aab'>Jul '24 – Jun '25</b><br>
        Currency &nbsp;·&nbsp; <b style='color:#aab'>INR (₹, Lakh/Crore)</b>
...
# ═══════════════════════════════════════════════════════════
kpis = [
    ("revenue","Total Revenue",   inr(T_REV,1),      "#ff3d8a"),
    ("trend-up","Avg ROI",        f"{AVG_ROI:.2f}x",  "#a78bfa"),
    ("target","Conversions",      num(T_CONV,2),      "#60a5fa"),
    ("wallet","Ad Spend",         inr(T_SPEND,1),     "#fb923c"),
    ("eye","Impressions",         num(T_IMP,1),       "#34d399"),
    ("cursor","Avg CTR",          f"{AVG_CTR:.2f}%",  "#f9a8d4"),
    ("bolt","ROAS",               f"{ROAS:.1f}x",     "#fcd34d"),
    ("magnet","Total Leads",      num(T_LEADS,2),     "#6ee7b7"),
]

cols = st.columns(8)
for col,(ic,lbl,val,clr) in zip(cols,kpis):
    with col:
        st.markdown(f"""
        <div style='background:#0d0d1a;border:1px solid #1a1a2e;border-radius:16px;
          ...
          <div style='margin-bottom:9px'>{icon(ic,clr,size=17,stroke=1.9,box=34)}</div>
          <div style='font-family:Sora,sans-serif;font-size:1.22rem;font-weight:800;
...
with t1:
    ...
    with c1:
        ...
        for _,r in ch_s.iterrows():
            fig.add_trace(go.Bar(
                ...
                text=f"  {inr(r['Revenue'],1)}",
                textposition='outside',
                textfont=dict(color='#ccccdd',size=11),
                hovertemplate=f"<b>{r['Campaign_Type']}</b><br>Revenue: {inr(r['Revenue'],2)}<br>ROI: {r['ROI']:.2f}x<extra></extra>",
            ))
    ...
    with c2:
        ...
        fig.update_layout(**{k:v for k,v in base_layout(h=320).items() if k not in('xaxis','yaxis')},
                          title="Revenue Share",
                          showlegend=False,
                          annotations=[dict(text=f'{inr(T_REV,1)}<br>Total',
    ...
    with c4:
        ...
        for i,(stage,tot,pct,clr) in enumerate(zip(stages,totals,pcts,clrs)):
            fig.add_trace(go.Bar(
                ...
                hovertemplate=f"<b>{stage}</b><br>Count: {indian_grouping(tot)}<br>vs Impressions: {pct:.1f}%<extra></extra>",
            ))
    ...
    disp = ch.copy()
    disp['Revenue']  = disp['Revenue'].map(inr_full)
    disp['Spend']    = disp['Spend'].map(inr_full)
    disp['Profit']   = disp['Profit'].map(inr_full)
    ...
    disp['CPA']      = disp['CPA'].map(lambda v: f"₹{indian_grouping(v)}")
    ...
    with c1:
        ...
        for ch_name, grp in fdf.groupby('Campaign_Type'):
            fig.add_trace(go.Violin(
                ...
                fillcolor=hex_to_rgba(fpal[ch_name], 0.2),
    ...
    with c2:
        ...
        for name,clr,key in [('Revenue','#ff3d8a','Revenue'),
            ...
            fig.add_trace(go.Bar(
                ...
                text=ch[key].map(lambda v: inr(v,1)),
                textposition='outside',textfont=dict(size=9,color='#aaaacc')
            ))
...
with t3:
    ...
    pvt_text = np.array([[inr(v,1) for v in row] for row in pvt.values])
    fig = go.Figure(go.Heatmap(
        ...
        texttemplate='%{text}',
        textfont=dict(size=11,color='#eeeeff'),
        customdata=np.array([[inr_full(v) for v in row] for row in pvt.values]),
        hovertemplate='Segment: %{y}<br>Channel: %{x}<br>Revenue: %{customdata}<extra></extra>',
        colorbar=dict(tickfont=dict(color='#7777aa'),
...
with t4:
    ...
    for ch_name,grp in monthly.groupby('Campaign_Type'):
        ...
        fig.add_trace(go.Scatter(
            ...
            customdata=grp['Revenue'].map(inr_full),
            hovertemplate=f'<b>{ch_name}</b><br>Month: %{{x}}<br>Revenue: %{{customdata}}<extra></extra>'
        ))
    ...
    with c2:
        ...
        for ch_name,grp in monthly.groupby('Campaign_Type'):
            ...
            fig.add_trace(go.Scatter(
                ...
                fillcolor=hex_to_rgba(clr, 0.18),
                stackgroup='one',
            ))
        fig.update_layout(**base_layout(h=340),
    ...
    for ch_name in sorted(fdf['Campaign_Type'].unique()):
        ...
        fig.add_trace(go.Bar(
            ...
            customdata=sub['Revenue'].map(inr_full),
            hovertemplate=f'<b>{ch_name}</b><br>Quarter: %{{x}}<br>%{{customdata}}<extra></extra>'
        ))
...
with t5:
        ...
        with (c1 if i%2==0 else c2):
            ...
            <div style='background:#0d0d1a;border:1px solid #1a1a2e;border-left:3px solid {clr};
              ...
              <div style='display:flex;align-items:center;gap:10px;margin-bottom:8px'>
                {icon(ic,clr,size=16,stroke=1.9,box=32)}
    ...
    recs = [
        ("#ff3d8a","trend-up",f"Double Down on {best_ch}",
        ...
        ("#a78bfa","users",f"Segment-First Planning for {best_seg}",
        ...
        ("#60a5fa","globe",f"Scale {best_lang} Localisation",
        ...
        ("#34d399","refresh",f"Revive {worst_ctr} with Creative Overhaul",
        ...
        ("#fb923c","calendar","Seasonality-Driven Budget Shifts",
        ...
        ("#f9a8d4","chart","ROI-Gated Bidding Rules",
        ...
        ("#fcd34d","link","Influencer × Paid Ads Whitelisting",
        ...
        ("#6ee7b7","tools","Build a Marketing Mix Model (MMM)",
        ...
        <div style='background:linear-gradient(135deg,#0d0d1a,#0a0a14);
          ...
          <div>
            <div style='font-family:Sora,sans-serif;font-size:0.9rem;font-weight:700;
              color:#eeeeff;margin-bottom:5px'>{title}</div>
...
<div style='text-align:center;padding:1rem 0;color:#2a2a45;font-size:0.75rem;
  ...
  <span style='display:inline-flex;vertical-align:middle;margin-right:6px'>{icon('bag','#555880',size=13,stroke=2,bg=False)}</span>
  <b style='color:#333355'>Nykaa Marketing Intelligence</b>&nbsp; · &nbsp;
  Powered by Streamlit & Plotly &nbsp; · &nbsp;
  Dataset: Jul '24 – Jun '25 &nbsp; · &nbsp;
  <span style='color:#ff3d8a'>{indian_grouping(len(fdf))} campaigns analysed</span>
</div>
""", unsafe_allow_html=True)
